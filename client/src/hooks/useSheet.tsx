...
type ISheetContext = {
  ...
  getClickedCellAndRow:any|null;
  showFormulaCells:(cells:string[])=>void;
  changeTextOnFormula:(Q:Quill,cell:ICellDetail) => void;
  convertVirToCellId:(value:string[])=>any;
...
export const SheetProvider = ({ children }: ISheetProviderProps) => {
 ...
 type UserFriendInterfaceType={
  virtualId:string,
  cellId:string
 }|""
  const getClickedCellAndRow= useMemo((): UserFriendInterfaceType => {
          
          if (!selectedCell?.columnId) return "";
          return {
            virtualId:`${convertToTitle(selectedCell.columnId)}${selectedCell.rowId}`,
  ...
  function convertVirToCellId(cells: string[]): any {
    // console.log(cell)
    // Extract the column letters and row numbers using regular expressions
    const resultant=[];
    ...
    // Convert column letters to a numeric column index (A=1, B=2, ..., Z=26, AA=27, etc.)
    let columnNumber = 0;
    ...
    // Return the row and column as a string in "row,column" format
    resultant.push(`${columnNumber},${rowNumber}`);}
    return getAllrelativeDataOfGivenCellIds(resultant);
  }
  function getAllrelativeDataOfGivenCellIds(cells: string[]): any {
    const mainresult=[];
    for(let cell of cells){
      const CellObject=getCellById(cell);
      let curVal="";
      CellObject?.content?.forEach(element => {
        // console.log(element);
        curVal+=element.insert.replace(/\n/g,'');
    });
     mainresult.push(curVal); 
    }
    return {cellIds:cells,cellsData:mainresult};
  }
  ...
  // Loved One
  const showFormulaCells = (cells: string[]): void => {
    const Jells: any = cells.map((cell) => getCellById(cell)?._id); // Use map to transform the array
    
    setHighLightCells((e)=>{
      
      return Jells});
    setActiveHighLightIndex(Jells.length ? 0 : null);
};
  const changeTextOnFormula=async(quill:Quill,newCell:ICellDetail)=>{
    if (!editCell || !gridId) return;
    try {
      let text = newCell.text;
      const content: any[]|undefined = newCell?.content;
